...
namespace DAL
{
	public class SearchDb
	{
		...
		{
			const string query = "SELECT * FROM dbo.AspNetUsers WHERE EmailConfirmed = @id";
		...
		private void AppendTags(Project p)
		{
			...
			const string query = "SELECT * FROM Project_tags JOIN Tags ON (Project_tags.Project_ID=@projectid AND Project_tags.Tag_ID = Tags.ID);";
			using (var con = new SqlConnection(ConnectionString))
			{
				using (var sqlcommand = new SqlCommand(query, con))
				{
